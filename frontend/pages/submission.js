...
// ── Dynamic API base (works locally AND on Render/Netlify) ──
// If your frontend is served from the same origin as backend, use ''.
// If frontend is on Netlify and backend is on Render, set your Render URL here.
const BACKEND_ORIGIN = (function () {
  const h = window.location.hostname;
  // Running locally
  if (h === 'localhost' || h === '127.0.0.1') return 'http://localhost:3000';
  // On Render — replace with your actual Render backend URL
  return 'https://internsync-backend.onrender.com'; // ← UPDATE THIS if needed
})();
...
function statusBadge(status) {
  ...
  if (s === 'reviewed')return '<span class="sub-badge badge-reviewed">Reviewed</span>';
...
uploadZone.addEventListener('click', (e) => {
  if (!e.target.closest('#removeFile')) getFileInput().click();
});
...
document.getElementById('removeFile').addEventListener('click', e => {
  e.stopPropagation(); resetUploadZone();
});
...
document.getElementById('submitBtn').addEventListener('click', async () => {
  ...
  if (!internId || internId === 'null' || internId === 'undefined') {
    return showToast('Please log in as an intern first.', true);
  }
  ...
  try {
    ...
    // Add to history table immediately (no page reload needed)
...
async function loadHistory() {
  ...
  if (!internId || internId === 'null' || internId === 'undefined') {
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:36px;">Log in to view history.</td></tr>`;
    return;
  }
  ...
  try {
    ...
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--red);padding:36px;">Failed to load history. Check server connection.</td></tr>`;
  }
}

function buildRow(r) {
  // Build correct file URL using dynamic BACKEND_ORIGIN
  const fileUrl = r.file_path
    ? `${BACKEND_ORIGIN}/${r.file_path.replace(/^[.\/\\]+/, '')}`
    : null;

  const fileCell = fileUrl
    ? `<a href="${escHtml(fileUrl)}" target="_blank" rel="noopener" class="tbl-btn tbl-btn-view">View File</a>`
    : r.github_url
      ? `<a href="${escHtml(r.github_url)}" target="_blank" rel="noopener" class="tbl-btn tbl-btn-view">GitHub</a>`
      : `<span style="color:var(--muted)">—</span>`;

  const fbCell = r.feedback
    ? `<button class="tbl-btn tbl-btn-fb view-fb-btn" data-fb="${encodeURIComponent(r.feedback)}">View</button>`
